extends Node2D
## Cutscene I - mordida do vampiro. Sem narrador, apenas animacao e legendas.
...
func _beats() -> void:
	await _subtitle_hold("[i]Capitulo I - A Mordida[/i]", 2.35)
	if not _running:
		return

	await _subtitle_hold("Estas na sala. A noite acaba de cair...", 2.65)
	if not _running:
		return
	...
	await _subtitle_hold(
		"A mordida queima nas veias. Ja nao es so humano - es vampiro.\n" +
