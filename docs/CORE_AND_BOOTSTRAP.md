...
3. `bootstrap.gd`: sinal `application_ready` do autoload **App** → **Game**.`notify_bootstrap_handoff()` → **App**.`go_to_scene(first_play_scene)`. Cadeia por defeito: **intro** (Cap. I) → **ritual_awakening** (Cap. II) → **main**.
